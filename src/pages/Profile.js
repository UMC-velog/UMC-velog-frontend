...
const SERVER_URL = `http://43.200.183.201:8080/boards`;
...
const Profile = () => {
    ...
    useEffect(() => {
        axios.get([SERVER_URL, `${userId}/profiles`].join("/"))
        .then((res) => {
            setUsername(res.data[0].username);                
            setBoards(res.data[0].boards.map((item) =>({
                ...item,
